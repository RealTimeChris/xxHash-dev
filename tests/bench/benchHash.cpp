 ...
 */
static double bench_hash_internal(BMK_benchFn_t hashfn, BMK_benchFn_t payload, size_t nbBlocks, sizeFunction_f selectSize, size_t& size, unsigned total_time_ms,
	unsigned iter_time_ms, size_t& newSize) {
...
#define NB_BLOCKS_MAX 1024
	...
	for (size_t n = 0; n < nbBlocks; n++) {
		...
		newSize			 = srcSizes[n];
		dstBuffers[n]	 = static_cast<void*>(dstBuffer_static);
		dstCapacities[n] = dstSize;
	}


	BMK_benchParams_t params = { .benchFn = hashfn,
...
static size_t rand_1_N(size_t N) {
	return (( size_t )rand() % N) + 1;
}

static size_t identity(size_t s) {
	return s;
}

static size_t benchLatency(const void* src, size_t srcSize, void* dst, size_t dstCapacity, void* customPayload) {
	( void )dst;
	( void )dstCapacity;
...
double bench_hash(BMK_benchFn_t hashfn, BMK_benchMode benchMode, size_t& size, BMK_sizeMode sizeMode, unsigned total_time_ms, unsigned iter_time_ms,size_t& newSize) {
	...
	if (nbBlocks > NB_HASH_ROUNDS_MAX)
		nbBlocks = NB_HASH_ROUNDS_MAX;
	auto newValue = bench_hash_internal(benchfn, payload, nbBlocks, sizef, size, total_time_ms, iter_time_ms, newSize);
	return newValue;
}

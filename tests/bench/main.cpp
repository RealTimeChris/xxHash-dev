 ...
 */
static bool isCommand(const char* string, const char* longCommand) {
	assert(string);
	assert(longCommand);
...
static int hashID(const char* hname) {
	...
	for (id = 0; id < NB_HASHES; id++) {
		...
		if (strlen(hname) != strlen(hashCandidates[id].name))
			continue;
...
inline void executePythonScript(const std::string& scriptPath, const std::string& argument01, const std::string& argument02) {
	std::string command = "python " + scriptPath + " " + argument01 + " " + argument02;
...
inline static std::string urlEncode(const jsonifier::string& value) {
...
size_t countDistinctLibraryNames(const std::vector<benchmark_result_final_parse>& results) {
	std::unordered_set<std::string> libraryNames;

	for (const auto& result: results) {
		libraryNames.insert(result.libraryName.operator std::string());
	}

	return libraryNames.size();
}

inline static std::string generateMarkdown(const benchmark_suite_results& results, std::string githubRepoAndPath) {
	std::ostringstream markdownStream{};
	jsonifier::vector<benchmark_result_final_parse> resultsNew{};
	size_t distinctLibraryNames{ countDistinctLibraryNames(results.results) };
	markdownStream << "# Benchmark Results: " + results.benchmarkSuiteName.operator jsonifier::string_base<char, 0ull>() << std::endl;
	std::string currentTestName{};
	uint64_t currentIndex{};
	if (distinctLibraryNames > 0) {
		for (size_t i = 0; i < results.results.size() / distinctLibraryNames; ++i) {
			const auto& result = results.results[i];
			if (currentTestName != result.benchmarkName || i == results.results.size() - 1) {
				resultsNew.emplace_back(results.results[i]);
			}
		...
		for (auto& value: resultsNew) {
			...
			// Generate the section header and image link
			markdownStream << "### " << currentIndex + 1 << ". " << value.benchmarkName << std::endl;
			markdownStream << "<p align=\"left\"><img src=\"" + githubRepoAndPath << encodedBenchmarkName << "_results.jpg?raw=true\"width=\"400\"/></p>\n\n";
			++currentIndex;
		}
	} else {
		return {};
...
int main(int argc, const char* argv[]) {
	...
	for (arg_nb = 1; arg_nb < argc; arg_nb++) {
		...
		if (isCommand(*arg, "--list")) {
			return display_hash_names();
		}
		if (longCommandWArg(arg, "--n=")) {
			nb_h_test = readIntFromChar(arg);
			continue;
		} /* hidden command */
		if (longCommandWArg(arg, "--minl=")) {
			largeTest_log_min = readIntFromChar(arg);
			continue;
		}
		if (longCommandWArg(arg, "--maxl=")) {
			largeTest_log_max = readIntFromChar(arg);
			continue;
		}
		if (longCommandWArg(arg, "--mins=")) {
			smallTest_size_min = ( size_t )readIntFromChar(arg);
			continue;
		}
		if (longCommandWArg(arg, "--maxs=")) {
			smallTest_size_max = ( size_t )readIntFromChar(arg);
			continue;
		}
	...
	std::string newPath{ ROOT_PATH };
	benchmark_suite_results testResults{};
	...
	if (smallTest_size_max >= smallTest_size_min) {
		//bench_throughput_smallInputs(hashCandidates + hashNb, nb_h_test, smallTest_size_min, smallTest_size_max, testResults);
		bench_throughput_randomInputLength(hashCandidates + hashNb, nb_h_test, smallTest_size_min, smallTest_size_max, testResults);
		//bench_latency_smallInputs(hashCandidates + hashNb, nb_h_test, smallTest_size_min, smallTest_size_max, testResults);
		//bench_latency_randomInputLength(hashCandidates + hashNb, nb_h_test, smallTest_size_min, smallTest_size_max, testResults);
	}
	file_loader jsonData{ newPath + "/BenchmarkData.json" };
	jsonData.saveFile(static_cast<std::string>(parser.serializeJson(testResults)));
	executePythonScript(newPath + "/GenerateGraphs.py", newPath + "/BenchmarkData.json", newPath + "/Graphs");
	file_loader mdData{ newPath + "./BenchmarkData.md " };
	mdData.saveFile(generateMarkdown(testResults, "https://github.com/RealTimeChris/xxHash-dev/blob/main/tests/bench/Graphs/"));

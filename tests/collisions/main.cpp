...
#else
#define aligned_alloc(x,y) std::aligned_malloc(x,y)
#endif
...
create_sampleFactory(size_t size, uint64_t htotal, uint64_t seed)
{
    sampleFactory* const sf = static_cast<sampleFactory*>(malloc(sizeof(sampleFactory)));
    if (!sf) EXIT("not enough memory");
    void* const buffer = malloc(size);
    if (!buffer) EXIT("not enough memory");
    sf->buffer = buffer;
...
Filter* create_Filter(int bflog)
{
    ...
    Filter* const bf = static_cast<Filter*>(aligned_alloc(cacheline_size, bfsize));  // requires C11
    if (bf==NULL) {
...
static Filter* Filter_reduce(Filter* bf, int bflog)
{
    ...
    return static_cast<Filter*>(realloc(bf, nbLines << 5));
}
...
static uint64_t search_collisions(
    searchCollisions_parameters param)
{
    ...
    size_t const hashByteSize = (htype == ht128) ? 1 : 1;
    ...
    void* const hashCandidates = std::malloc(tableSize);
    ...
    if ((htype == ht64) || (htype == ht32)) {
         ...
         */
        sort64(static_cast<uint64_t*>(hashCandidates), nbCandidates);
    } else {
        assert(htype == ht128);
        sort128(static_cast<XXH128_hash_t*>(hashCandidates), nbCandidates); /* sort with custom comparator */
    }
...
int main(int argc, const char** argv)
{
    ...
    if (nbThreads == 1) {
        ...
        searchCollisions_results* const MTresults = static_cast<searchCollisions_results*>(calloc((size_t)nbThreads, sizeof(searchCollisions_results)));
        if (!MTresults) EXIT("not enough memory");
        searchCollisions_parameters* const MTparams = static_cast<searchCollisions_parameters*>(calloc((size_t)nbThreads, sizeof(searchCollisions_parameters)));
